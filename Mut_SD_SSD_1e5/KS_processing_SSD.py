# Short script to process the KS statistics for the later construction of Supplementary Fig. 2
import os
import glob
import pandas as pd
...
for directory in iters:
    os.chdir(f'F:/Simon_Aube/Hausser_2019_reanalysis/Simulations_ADS_Hausser/pythonProject/'
             f'Final_simulations_v2/Mut_SD_SSD_1e5/{directory}')
...
os.chdir(f'F:/Simon_Aube/Hausser_2019_reanalysis/Simulations_ADS_Hausser/pythonProject/Final_simulations_v2/Mut_SD_SSD_1e5')
...
# Only the comparison with SSD couples
KS_all = KS_all[KS_all['Comparison'] == 'SSD'].copy().reset_index(drop=True)
...
KS_sub.to_csv('KS_modified_SSD_1e5.csv')
...
# The dataframe is exported as csv to be used in the Colaboratory notebook where the Supp. figure 3 is generated
KS_means.to_csv('KS_means_SSD_1e5.csv')